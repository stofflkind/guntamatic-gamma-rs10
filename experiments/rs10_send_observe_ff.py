...
def main():
    ...
    ap.add_argument("--interleave-ff", action="store_true",
                    help="send physical BYTE FF BYTE FF ... pattern")
    args = ap.parse_args()

    tx = make_mode_frame(args.mode)
    wire = b"".join(bytes((b, 0xFF)) for b in tx) if args.interleave_ff else tx
    ...
    print(f"wire mode : {'interleaved FF' if args.interleave_ff else 'logical bytes'}")
